...
import { avisarCorridaAssumida } from '@/lib/taxidog-rotas'
...
interface Props {
  ...
  caminho: string
  // Página de rotas (link do "Rota #N").
  caminhoRotas: string
  hojeISO: string
  corridas: CorridaDetalhe[]
  rotaPorCorrida: Record<string, RotaDaCorrida>
  taxidogs: { id_funcionario: string; nome: string }[]
  // Gestão (dono, administrador, gestão de agendamentos) — mesma regra de
...
export default function TaxiDogPainel({ data, caminho, caminhoRotas, hojeISO, corridas, rotaPorCorrida, taxidogs, podeAtribuir, podeAssumir = false, modoMotorista = false }: Props) {
  ...
  const seletorDe = (c: CorridaDetalhe) => (
    <SeletorTaxiDog
      c={c}
      podeAtribuir={podeAtribuir && !rotaPorCorrida[c.id_corrida]}
      taxidogs={taxidogs}
      disabled={isPending}
                  ...
                  >
                    ...
                    {rotaPorCorrida[c.id_corrida] ? (
                      <div className="kanban-card-acao" onClick={e => e.stopPropagation()}>
                        <Link href={`${caminhoRotas}?rota=${rotaPorCorrida[c.id_corrida].id_rota}`} className="btn btn-secondary btn-sm">
                          Ver Rota #{rotaPorCorrida[c.id_corrida].numero}
                        </Link>
                      </div>
                    ) : (
                      <AcaoDoCard
                        ...
                        onAssumir={() => {
                          avisarCorridaAssumida(c.id_corrida)
...
function DetalheCorrida({
  corrida: c, rota, caminhoRotas, erro, isPending, podeAtribuir, mostrarEtapas, confirmandoCancelamento, seletor,
  ...
  // Corrida de rota anda pela rota (tela da rota), não por aqui.
  const acao = mostrarEtapas && !rota ? proximaAcaoCorrida(c.status, c.modalidade) : null
      ...
      <div className="modal" style={{ maxWidth: 560 }} onClick={e => e.stopPropagation()}>
        ...
        <div className="modal-body">
          ...
          {rota && (
            <div className="dash-detail-row">
              <span>Rota</span>
              <span><Link href={`${caminhoRotas}?rota=${rota.id_rota}`} className="text-accent">Rota #{rota.numero}</Link></span>
            </div>
          )}
        ...
        <div className="modal-footer" style={{ justifyContent: 'space-between', flexWrap: 'wrap' }}>
          <div className="flex gap-2">
            <a href={linkRota(c)} target="_blank" rel="noopener noreferrer" className="btn btn-secondary btn-sm">
              <IconRoute style={{ width: 14, height: 14 }} /> Abrir no Google Maps
            </a>
          ...
          {mostrarEtapas && !acao && !rota && c.status === 'entregue_loja' && (
            <span className="text-sm text-muted">Aguardando o serviço terminar para a entrega</span>
          )}
          {mostrarEtapas && !acao && !rota && c.status === 'agendada' && c.modalidade === 'entregar' && (
