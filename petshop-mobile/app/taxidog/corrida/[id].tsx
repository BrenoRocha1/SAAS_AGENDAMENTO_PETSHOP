...
import { buscarCorrida, type RotaDaCorrida } from '@/hooks/useMinhasCorridas'
...
export default function CorridaDetalheScreen() {
  ...
  // Corrida numa rota (migration 053) só anda pela tela da rota.
  const [rota, setRota] = useState<RotaDaCorrida | null>(null)
  const [loading, setLoading] = useState(true)
  const [enviando, setEnviando] = useState(false)
  const [erro, setErro] = useState<string | null>(null)

  const carregar = useCallback(async () => {
    const r = await buscarCorrida(id, hojeBrasilISO())
  ...
  const acao = aguardandoAceite || semTaxiDog || rota ? null : proximaAcaoCorrida(c.status, c.modalidade)
  ...
  return (
    <ScreenContainer onRefresh={carregar} refreshing={false}>
      ...
      <View style={styles.acoes}>
        ...
        {semTaxiDog && !aguardandoAceite && !rota && (
          <Pressable
            style={({ pressed }) => [styles.botaoAcao, (pressed || enviando) && { opacity: 0.85 }]}
            disabled={enviando}
            onPress={assumir}
...
const styles = StyleSheet.create({
  ...
  bannerRotaTitulo: { ...typography.heading.sm, color: colors.primary600 },
  bannerRotaTexto: { ...typography.body.md, color: colors.textDim },
